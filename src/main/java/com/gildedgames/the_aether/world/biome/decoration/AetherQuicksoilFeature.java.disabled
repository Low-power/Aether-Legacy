...
public class AetherQuicksoilFeature extends WorldGenerator {

	public AetherQuicksoilFeature() {
		super();
	}
		...
		for (int x1 = x - 3; x1 < x + 4; x1++) {
			for (int z1 = z - 3; z1 < z + 4; z1++) {
				if (world.getBlock(x1, y, z1) == Blocks.air && ((x1 - x) * (x1 - x) + (z1 - z) * (z1 - z)) < 12) {
					setBlockAndNotifyAdequately(world, x1, y, z1, AetherBlocks.quicksoil, 0);
				}
			}
		}
		return true;
