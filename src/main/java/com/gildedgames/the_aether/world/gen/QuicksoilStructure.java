...
public class QuicksoilStructure extends MapGenBase {
		...
		for (int x = 3; x < 12; x++) {
			for (int z = 3; z < 12; z++) {
				for (int n = 3; n < 48; n++) {
					...
					if(blocks[pos] == Blocks.air && blocks[(z * 16 + x) * 128 + (n + 1)] == AetherBlocks.aether_grass && blocks[(z * 16 + x) * 128 + (n + 2)] == Blocks.air) {
						generate(blocks, x, n, z);
						n += 128;
					}
				}
			}
		...
		for (int x = posX - 3; x < posX + 4; x++) {
			for (int z = posZ - 3; z < posZ + 4; z++) {
				...
				if(blocks[position] == Blocks.air && ((x - posX) * (x - posX) + (z - posZ) * (z - posZ)) < 12) {
					blocks[position] = AetherBlocks.quicksoil;
				}
			}
		}
	}
