...
public class LargeColdAercloudComponent extends AetherStructure {

	private NBTTagCompound data = new NBTTagCompound();

	private int x_tendency, z_tendency;

	public LargeColdAercloudComponent() {
	}

	public LargeColdAercloudComponent(Random random, int chunkX, int chunkY, int chunkZ) {
		this.coordBaseMode = 0;
		this.x_tendency = random.nextInt(3) - 1;
		this.z_tendency = random.nextInt(3) - 1;
	...
	public boolean generate() {
		...
		for (int n = 0; n < 64; ++n) {
			...
			x += icd.getByte("xOffset") + this.x_tendency;

			if (icd.hasKey("yOffset")) {
				y += icd.getByte("yOffset");
			}

			z += icd.getByte("zOffset") + this.z_tendency;
			...
			for (int x1 = x; x1 < x + xMax; ++x1) {
				for (int y1 = y; y1 < y + yMax; ++y1) {
					for (int z1 = z; z1 < z + zMax; ++z1) {
						if(getBlockStateWithOffset(x1, y1, z1) == Blocks.air) {
							if (Math.abs(x1 - x) + Math.abs(y1 - y) + Math.abs(z1 - z) < 12 + shapeOffset) {
								setBlockWithOffset(x1, y1, z1, AetherBlocks.aercloud, 0);
							}
						}
					}
				}
