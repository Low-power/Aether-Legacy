	...
	public void harvestBlock(World world, EntityPlayer player, int x, int y, int z, int meta) {
		...
		if (crateType == 0) {
			...
			if (world.rand.nextInt(9) == 0) {
				this.dropBlockAsItem(world, x, y, z, new ItemStack(AetherItems.candy_cane_sword));
			} else {
				for (int size = 1; size <= randomNumber; ++size) {
					this.dropBlockAsItem(world, x, y, z, new ItemStack(AetherItems.gingerbread_man));
				}
			}
		} else {
			...
			world.playSoundAtEntity(present, "game.tnt.primed", 1F, 1F);
		}
	}
