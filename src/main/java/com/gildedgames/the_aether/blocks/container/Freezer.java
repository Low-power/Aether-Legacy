...
public class Freezer extends BlockAetherContainer {
	...
	public void registerBlockIcons(IIconRegister registry) {
		...
		this.top_icon = registry.registerIcon(Aether.find("freezer_top"));
	...
	public Item getItemDropped(int meta, Random rand, int fortune_level) {
		return Item.getItemFromBlock(AetherBlocks.freezer);
	}
	...
	public void randomDisplayTick(World world, int x, int y, int z, Random random) {
		if (world.getBlockMetadata(x, y, z) == 1) {
			...
			world.spawnParticle("snowshovel", fx, fy, fz, 0D, 0D, 0D);
		}
	}
	...
	public void breakBlock(World world, int x, int y, int z, Block block, int meta) {
		...
		if (tileentity instanceof FreezerTileEntity) {
			FreezerTileEntity freezer_tile = (FreezerTileEntity)tileentity;
			for (int i = 0; i < freezer_tile.getSizeInventory(); i++) {
				ItemStack itemstack = freezer_tile.getStackInSlot(i);
				if (itemstack != null) {
