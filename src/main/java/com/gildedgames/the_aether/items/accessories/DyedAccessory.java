...
public class DyedAccessory extends Accessory {

	private IIcon overlayIcon;
	private IIcon emptySlotIcon;

	public DyedAccessory(AccessoryType type) {
		super(type);
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int facing, float hitX, float hitY, float hitZ) {
		...
		if (world.getBlock(x, y, z) == Blocks.cauldron) {
			if (heldItem != null && heldItem.getItem() instanceof DyedAccessory) {
				int j1 = BlockCauldron.func_150027_b(world.getBlockMetadata(x, y, z));
				if (j1 > 0) {
					((DyedAccessory)heldItem.getItem()).removeColor(heldItem);
					Blocks.cauldron.func_150024_a(world, x, y, z, j1 - 1);
					return true;
				}
			}
		}
