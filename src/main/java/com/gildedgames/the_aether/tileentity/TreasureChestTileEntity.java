...
public class TreasureChestTileEntity extends TileEntityChest {
	...
	public void unlock(int kind) {
		...
		if (kind == 1) {
			for (p = 0; p < 5 + random.nextInt(1); ++p) {
				this.setInventorySlotContents(random.nextInt(this.getSizeInventory()), SilverDungeonComponent.getSilverLoot(random));
			}
		}

		if (kind == 2) {
			for (p = 0; p < 5 + random.nextInt(1); ++p) {
				this.setInventorySlotContents(random.nextInt(this.getSizeInventory()), GoldenDungeonComponent.getGoldLoot(random));
			}
		}
