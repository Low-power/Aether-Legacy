...
public class Whirlwind extends EntityMob {

	public ArrayList<AetherParticle> particles;
	...
	public Whirlwind(World world) {
		...
		this.particles = new ArrayList<AetherParticle>();
	...
	public float getBlockPathWeight(int x, int y, int z) {
		return this.worldObj.getBlock(x, y - 1, z) == AetherBlocks.aether_grass ? 10F : this.worldObj.getLightBrightness(x, y, z) - 0.5F;
	}
	...
	protected void applyEntityAttributes() {
		...
		getEntityAttribute(SharedMonsterAttributes.movementSpeed).setBaseValue((this.rand.nextDouble() * 0.025D) + 0.025D);
	...
	public void onLivingUpdate() {
		...
		if(player != null && player.onGround && isEvil()) {
		...
		if (!this.worldObj.isRemote) {
			...
			if (this.actionTimer >= 128) {
				if(isEvil()) {
					...
					creeper.setLocationAndAngles(this.posX, this.posY + 0.5D, this.posZ, this.rand.nextFloat() * 360F, 0F);
					creeper.motionX = (double)(this.rand.nextFloat() - this.rand.nextFloat()) * 0.125D;
					creeper.motionZ = (double)(this.rand.nextFloat() - this.rand.nextFloat()) * 0.125D;
					this.worldObj.spawnEntityInWorld(creeper);
					this.actionTimer = 0;
					this.worldObj.playSoundAtEntity(this, "random.pop", 0.5F, 1F);
		...
		List<Entity> entity_list = (List<Entity>)this.worldObj.getEntitiesWithinAABBExcludingEntity(this, this.boundingBox.expand(2.5D, 2.5D, 2.5D));
			...
			if (d15 <= 1.5D + d17) {
				...
				if(entity instanceof Whirlwind) {
					...
					if(!isEvil()) {
						this.lifeLeft /= 2;
						setEvil(true);
						playSound("random.fizz", this.rand.nextFloat() - this.rand.nextFloat() * 0.2F + 1.2F, 1F);
					}
				}
			} else {
	...
	public boolean interact(EntityPlayer player) {
		...
		if (heldItem != null && heldItem.getItem() == Items.dye && AetherRankings.isRankedPlayer(player.getUniqueID())) {
			setColorData(heldItem.getItemDamage());
			return true;
		}
	...
	public void updateParticles() {
		if(!isEvil()) {
			int color = ItemDye.field_150922_c[this.getColorData()];
			for (int k = 0; k < 2; k++) {
				...
				if (this.isRainbow) {
					...
					float f1 = ((float) (this.ticksExisted % 25)) / 25F;
					particle.setRBGColorF(EntitySheep.fleeceColorTable[l][0] * (1F - f1) + EntitySheep.fleeceColorTable[i1][0] * f1, EntitySheep.fleeceColorTable[l][1] * (1F - f1) + EntitySheep.fleeceColorTable[i1][1] * f1, EntitySheep.fleeceColorTable[l][2] * (1F - f1) + EntitySheep.fleeceColorTable[i1][2] * f1);
				}
		...
		if(particles.size() > 0) {
			Iterator<AetherParticle> iterator = this.particles.iterator();
			while(iterator.hasNext()) {
				AetherParticle particle = iterator.next();
				if (particle.isDead) {
					iterator.remove();
					continue;
				}
	...
	public boolean getCanSpawnHere() {
		...
		return this.rand.nextInt(450) == 0 && this.worldObj.getBlock(x, y - 1, z) == AetherBlocks.aether_grass && this.worldObj.getBlockLightValue(x, y, z) > 8 && this.worldObj.checkNoEntityCollision(this.boundingBox) && this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox).isEmpty() && !this.worldObj.isAnyLiquid(this.boundingBox);
	}
