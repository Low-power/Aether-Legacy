...
public class Valkyrie extends AetherHostileMob {
	...
	public boolean interact(EntityPlayer player) {
		...
		if (stack != null && stack.getItem() == AetherItems.victory_medal && stack.stackSize >= 0) {
			if (stack.stackSize >= 10) {
				this.chatItUp(player, "Umm... that's a nice pile of medallions you have there...");
			} else if (stack.stackSize >= 5) {
				this.chatItUp(player, "That's pretty impressive, but you won't defeat me.");
			} else {
				this.chatItUp(player, "You think you're a tough guy, eh? Well, bring it on!");
			}
		} else {
			int line = rand.nextInt(3);

			if (line == 2) {
				this.chatItUp(player, "What's that? You want to fight? Aww, what a cute little human.");
			} else if (line == 1) {
				this.chatItUp(player, "You're not thinking of fighting a big, strong Valkyrie are you?");
			} else {
				this.chatItUp(player, "I don't think you should bother me, you could get really hurt.");
			}
		}
	...
	public void teleport(double x, double y, double z, int rad) {
		...
		if (!flag) {
			...
			this.moveForward = this.moveStrafing = this.rotationPitch = this.rotationYaw = 0F;
	...
	public void onUpdate() {
		...
		if (!this.onGround && this.getAttackTarget() != null && this.lastMotionY >= 0D && this.motionY < 0D && getDistanceToEntity(this.getAttackTarget()) <= 16F && canEntityBeSeen(this.getAttackTarget())) {
	...
	protected void dropFewItems(boolean var1, int var2) {
		dropItem(AetherItems.victory_medal, 1);
	}
