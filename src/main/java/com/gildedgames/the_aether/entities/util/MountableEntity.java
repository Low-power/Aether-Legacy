...
public abstract class MountableEntity extends AetherAnimal {
	...
	public void moveEntityWithHeading(float par1, float par2) {
		...
		if (entity instanceof EntityPlayer) {
			...
			float f = (float) (Math.atan2(d2, d01) * (180D / Math.PI)) - 90F;

			if (player.moveStrafing != 0F && player.worldObj.isRemote) {
				this.rotationYaw = this.updateRotation(this.rotationYaw, f, 40F);
			}
			...
			if (this.jumpPower > 0F && !this.isMountJumping() && (this.onGround || this.canJumpMidAir)) {
