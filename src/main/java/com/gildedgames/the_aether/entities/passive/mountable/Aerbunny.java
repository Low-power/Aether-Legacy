...
public class Aerbunny extends AetherAnimal implements IEntityAdditionalSpawnData {
	...
	public Aerbunny(World world) {
		...
		this.tasks.addTask(4, new EntityAIWatchClosest(this, EntityPlayer.class, 10F));
	...
	public void spawnExplosionParticle() {
		for (int i = 0; i < 5; ++i) {
			...
			double d3 = 10D;
			this.worldObj.spawnParticle("explode", this.posX + (double) (this.rand.nextFloat() * this.width * 2F) - (double) this.width - d0 * d3, this.posY + (double) (this.rand.nextFloat() * this.height) - d1 * d3, this.posZ + (double) (this.rand.nextFloat() * this.width * 2F) - (double) this.width - d2 * d3, d0, d1, d2);
		}
	}
	...
	public EntityAgeable createChild(EntityAgeable entity) {
		return new Aerbunny(this.worldObj);
	}
