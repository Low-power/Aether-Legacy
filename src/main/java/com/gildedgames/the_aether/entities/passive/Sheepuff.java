...
public class Sheepuff extends AetherAnimal {
	...
	protected void dropFewItems(boolean var1, int ammount) {
		if (!this.getSheared()) {
			this.entityDropItem(new ItemStack(Blocks.wool, 1 + this.rand.nextInt(2), this.getFleeceColor()), 0F);
		}
	}
	...
	public float getHeadRotationPointY(float p_70894_1_) {
		return this.sheepTimer <= 0 ? 0F : (this.sheepTimer >= 4 && this.sheepTimer <= 36 ? 1F : (this.sheepTimer < 4 ? ((float) this.sheepTimer - p_70894_1_) / 4F : -((float) (this.sheepTimer - 40) - p_70894_1_) / 4F));
	}

	@SideOnly(Side.CLIENT)
	public float getHeadRotationAngleX(float p_70890_1_) {
		if (this.sheepTimer > 4 && this.sheepTimer <= 36) {
			float f = ((float) (this.sheepTimer - 4) - p_70890_1_) / 32F;
	...
	protected void func_145780_a(int p_145780_1_, int p_145780_2_, int p_145780_3_, Block p_145780_4_) {
		this.playSound("mob.sheep.step", 0.15F, 1F);
	}
	...
	public boolean interact(EntityPlayer player) {
		...
		if (itemstack != null && itemstack.getItem() == Items.shears && !this.getSheared()) {
			if (!this.worldObj.isRemote) {
				...
				for (int j = 0; j < i; j++) {
					EntityItem item_entity = this.entityDropItem(new ItemStack(Blocks.wool, 1, this.getFleeceColor()), 1F);
					item_entity.motionY += this.rand.nextFloat() * 0.05F;
					item_entity.motionX += (this.rand.nextFloat() - this.rand.nextFloat()) * 0.1F;
					item_entity.motionZ += (this.rand.nextFloat() - this.rand.nextFloat()) * 0.1F;
				}
			}
	...
	public EntityAgeable createChild(EntityAgeable entity) {
		return new Sheepuff(this.worldObj);
	}
