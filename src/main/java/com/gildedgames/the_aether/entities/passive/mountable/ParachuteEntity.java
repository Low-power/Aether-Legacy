...
public class ParachuteEntity extends Entity implements IEntityAdditionalSpawnData {
	...
	public ParachuteEntity(World world, EntityPlayer player, boolean isGolden) {
		this(world);
	...
	public void spawnExplosionParticle() {
		if (this.worldObj.isRemote) {
			...
			double d3 = 10D;
			this.worldObj.spawnParticle("explode", this.posX + (double) (this.rand.nextFloat() * this.width * 2F) - (double) this.width - d0 * d3, this.posY - 0.5D + (double) (this.rand.nextFloat() * this.height) - d1 * d3, this.posZ + (double) (this.rand.nextFloat() * this.width * 2F) - (double) this.width - d2 * d3, d0, d1, d2);
		} else {
	...
	public void writeSpawnData(ByteBuf buffer) {
		...
		buffer.writeInt(this.ridingPlayer == null ? -1 : this.ridingPlayer.getEntityId());
	}

	@Override
	public void readSpawnData(ByteBuf buffer) {
		this.isGoldenParachute = buffer.readBoolean();
		int entity_id = buffer.readInt();
		if(entity_id == -1) this.ridingPlayer = null;
		else {
			Entity entity = this.worldObj.getEntityByID(entity_id);
			this.ridingPlayer = entity instanceof EntityPlayer ? (EntityPlayer)entity : null;
		}
	}
