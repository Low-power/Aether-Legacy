...
public class Moa extends EntitySaddleMount {
	...
	public Moa(World world, AetherMoaType type) {
		this(world);
	...
	public void onUpdate() {
		...
		if (!this.worldObj.isRemote && !this.isChild() && this.riddenByEntity == null) {
			if (this.secsUntilEgg > 0) {
				...
				this.playSound("mob.chicken.plop", 1F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1F);
				this.entityDropItem(MoaEgg.getStackFromType(this.getMoaType()), 0);
	...
	public void updateWingRotation() {
		...
		this.destPos += 0.2D;
		this.destPos = minMax(0.01F, 1F, this.destPos);
	...
	public void onMountedJump(float par1, float par2) {
		if (this.getRemainingJumps() > 0 && this.motionY < 0D) {
			if (!this.isOnGround()) {
				this.motionY = 0.7D;
				this.worldObj.playSoundAtEntity(this, "mob.bat.takeoff", 0.15F, MathHelper.clamp_float(this.rand.nextFloat(), 0.7f, 1.0f) + MathHelper.clamp_float(this.rand.nextFloat(), 0f, 0.3f));
	...
	public EntityAgeable createChild(EntityAgeable matingAnimal) {
		return new Moa(this.worldObj, this.getMoaType());
	}
